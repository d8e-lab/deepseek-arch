...
| **2** | 否决/纠正、确认过的 | 模型能看到 |
| **1** | **待观察**（模糊，等着被印证） | 看不到 |
| **0** | **待销毁**（只在"记忆装不下"时才会出现；闲置到期就归档/删除） | 看不到 |

**档位就是生命周期，没有第二套机制**：

```
 3 ──90 个活动日没用──▶ 2 ──90 个活动日没用──▶ 1 ──记忆装不下──▶ 0 ──再 180 活动日没用──▶ 归档/删除
   ◀──被用到 2 次（读全文/反复提到）──┘                 ◀──再次被用到 = 回到 1 重新观察
```

**关键：只是"没人用"不会致死。** 闲置最多降到 **1（待观察）** 就停住；只有**记忆总量超过上限**
（默认 400 条 = 可见 200 + 候选 200）时，才把最久没用的候选降到 0，再闲置 180 活动日才归档/删除。
所以你那些"一年才提一次"的偏好不会被悄悄清掉 —— 它会安静地待在观察区，等你下次提到它。

**关键：时间单位是"活动日"**——这一天你确实用了程序才算一天；你半年不开程序，这些天**不推进老化**，
所以不会一回来所有记忆同时"过期"被清空。

四个设计要点：

1. **"使用"只算两种**：模型真的 `memory_read` 读了全文，或者这条被写入/重申。**"出现在清单里"不算** —— 否则会变成"越注入越升级"的正反馈。（归纳代理为了查重读它算"看到"：能让它不被清掉，但不升级。）
2. **换出 ≠ 判死刑**：可见条目超过上限（默认 200 条）时，最久没用的降到 **1 观察区**，不是直接销毁。
3. **重新启用从 1 开始**：任何被清到 0 档的条目，再次被用到只回到 1（重新观察），要继续用才会回到 2/3 —— 不越级。
4. **能解释、能控制**：每次升降级/换出/销毁都进 `audit.jsonl`；`/memory show` 显示 `uses=N last-used=Nd ago pinned`；`/memory candidates` 分「待观察 / ⏳待销毁（已 N/180 活动日）」；`/memory pin <slug>` 永久免疫并把候选拉回可见；`/memory gc --dry-run` 先预览。

触发时机：**每次开新会话时结算一次**（在构建清单之前）+ `/memory gc` 手动。不常驻、不打扰、失败只写审计。

**候选区（`confidence` 0/1）怎么升上来**：那批条目 master 看不到、不会被注入、也永远不会去读它，
...
所以你不需要操心"模糊记忆会不会永远沉底"。（反之，长期没人提的候选条目会沿档位往下走：
1 →(90 活动日)→ 0 →(再 180 活动日)→ 归档，候选区不会变成无限期坟场。）
...
**"待销毁"在磁盘上长什么样？** 就是 **`confidence: 0`**（直接写在条目文件的 frontmatter 里）——
可以 grep、可以随目录迁移、语义自带。想人工查看：`/memory candidates` 分「待观察（confidence 1）/
⏳待销毁（confidence 0，已 N/180 活动日）」两段；`📌` 是用户钉住、免疫自动清理的条目。
