# Memory 机制 —— 讲解版（给用户看）

> 配套文件：
> - `plan/memory-heartbeat-design.md`（实施级设计稿，含代码行号依据；**§12 评审决议为准**）
> - `plan/claude-code-memory-reference.md`（Claude Code 记忆机制的调研报告，本方案的主要参照）
> 本文只讲「它会变成什么样、你如何使用、为什么这么设计」，不贴实现细节。

---

## 1. 一句话

在**不占用大量上下文**的前提下，让 agent 跨会话记住你的**偏好、约定与边界**，
并在相关任务时自动带上；记忆的写入由后台代理完成，任何失败都不影响你的正常使用。

---

## 2. 记忆长什么样（目录结构）

```
{workspace}/.deepseek-arch/memory/        # 项目层：只跟这个工作区有关
  MEMORY.md                              # 索引/概览：一行一条，注入 system prompt 的就是它
  reply-format.md                        # 主题文件：一个主题一个文件
  test-policy.md
  logs/2026/09/2026-09-13.md             # 追加式日志（可审计，不注入）
  state.json                             # 归纳游标等状态（增量判断用）
  audit.jsonl                            # 机器可读审计（谁在什么时候写了什么/为什么没注入）
  legacy/                                # 你手写的旧笔记（原样保留，不被程序改写）

~/.deepseek-arch/memory/                  # 全局层：跨项目通用偏好（结构同上）
```

**主题文件示例**（人可以直接读、直接改；格式对齐 Claude Code）：

```markdown
---
name: 回复格式偏好
description: 回复先给结论再给理由，不要长篇铺垫      # 召回判断相关性就看它，所以要具体
type: feedback                                    # user | feedback | project | reference
subject: reply.format                             # 合并键：同 subject 才可能被合并
tags: [reply, format, style]
scope: project
confidence: 3
signal: A
paths: ["src/**"]
created: 2026-09-12T09:10:00Z
updated: 2026-09-13T02:00:00Z
---

回复先给结论，再给理由；避免长篇铺垫。
**Why:** 用户明确说过「别铺垫」。
**How to apply:** 面向用户的总结，第一句就是结论。
```

**索引 `MEMORY.md` 示例**（注入 system prompt 的就是这段，一屏以内）：

```markdown
- [回复格式偏好](reply-format.md) — 回复先给结论再给理由，别铺垫 (confidence 3, updated today)
- [测试策略](test-policy.md) — 改动必须跑全量测试再汇报 (confidence 3, updated 3 days ago)
- [计划目录](plan-dir.md) — 计划文档写 {workspace}/.deepseek-arch/plan/ (confidence 2, updated 2 days ago)
```

设计取舍：**主题文件用 Markdown 而不是数据库/JSONL**，理由有三：
人是主要读者（你能直接改、能进 git、能 diff）；一个主题一个文件 → 「这条记忆何时更新」由文件 mtime 天然表达；
agent 对项目层可以直接用普通 `read_file` 读全文（全局层要用专用工具，见 §5）。
格式与目录层级**刻意对齐 Claude Code**（`MEMORY.md` 索引 + `logs/yyyy/mm/` + `name/description/type` frontmatter + 正文
`规则` · `**Why:**` · `**How to apply:**`），以便复用其提示词约束与经验。

---

## 3. 三个时刻：注入 / 写入 / 读取

### 3.1 注入（把记忆放进模型上下文）

> 决定（2026-09-13）：**概览进 system prompt**，参照 Claude Code 的做法；
> **不再使用「临时 user 消息」**形态（设计稿 §12 R11/R12）。

| 时刻 | 注入什么 | 为什么 |
|:--|:--|:--|
| **会话创建 / resume 首轮** | **清单**（`MEMORY.md` 的全部行，含文件名 + 一句话描述 + 置信度 + 更新时间；受预算约束，超预算时用 `deepseek-v4-flash` 挑相关行）追加进 system prompt | 让模型一开始就知道「有哪些方面的记忆」，并能自己去读全文 |
| **compact 之后** | 用当前记忆**重建** system prompt（并同步重写会话快照文件） | compact 本来就要换前缀，这次重建不额外损失缓存；顺便让过期清单变新 |
| **会话中清单发生变化** | 默认**不刷新 system prompt**（避免整段历史重算）；变化以**落盘的一条提示消息**注入（UI 只渲染一行） | 注入落盘 → 历史字节稳定 → 前缀缓存不断（与 Claude Code 的附件落盘一致） |
| **你读过的记忆文件被更新** | 同上（一条落盘提示） | 避免模型基于旧内容下判断 |
| **想手动刷新** | `/memory refresh` | 显式动作：重建 system prompt + 重写快照（会作废该次请求的前缀，所以不做成自动） |

关键设计（为什么这样最省）：

- system prompt 在**一个会话内是冻结的**（现状：创建时构建 + 落盘快照，`resume` 复用快照，`src/core/session.ts:176-179, 191-194`）
  → 清单放进去**不会**产生任何缓存失效；唯一代价是「可能过期」。
- 「重装」只允许发生在**前缀本来就要变**的时刻：新会话（没有更长前缀）、compact（前缀被摘要替换）、或你手动 `/memory refresh`。
  **绝不在会话中途自动重装**——那一刻 system 变了，后面整段历史都要重算。
- 会变的内容（新增/改写/遗忘、你读过的文件被更新）**落盘成一条独立的提示消息**（UI 渲染为一行），
  这样它成为历史的一部分 → 下一轮前缀仍然命中（不落盘的话，每轮会把上一轮内容重算一次）。

### 3.2 写入（谁来记、什么时候记）

**你发消息 → 主 agent 正常干活（同时后台跑一个 memory agent）→ 你下一次发言时，上级新记忆以「变化提醒」生效。**

- memory agent 的输入：**上一轮的（你的消息 + agent 的最终回复）+ 本轮你的消息**，再加它自己按需检索出来的历史记忆。
  也就是它总是「拿着模型上一轮的输出 + 你这一轮的反馈」判断，因此：
  - 「不对，应该……」→ 能判定为**否决/纠正**；
  - 「就按这个做」→ 能判定为**方案确认**；
  - 它**看不到**工具调用轨迹与思维链（按你的要求），因此判定只依据对话文本。
- 它是**并发跑的**：独立中断信号，你 Ctrl+C 只中断主 agent；它跑失败/超时/卡住都只写日志，绝不打扰你。
- 同一时刻只有一个 memory agent 在跑（重复触发直接跳过，不排队）；两次归纳间隔 ≥30s。
- 写入配额：一次最多 3 条；宁少勿滥（技术事实、任务细节、一次性指令都不记）。

### 3.3 读取（模型想查更细的内容时）

- **清单已经在 system prompt 里**（`MEMORY.md` 的全部行，受预算约束）→ 模型知道有哪些记忆、各自一句话描述与文件名；
- 需要全文就用 **`memory_read` 工具**按路径读（项目层相对路径、全局层加 `global:` 前缀），
  它是 core 内直接读文件（绕过沙箱）→ 这是**全局层唯一**的读取通道（普通 `read_file` 受 `checkPath` 限制读不到 `~/.deepseek-arch/`）；
- **没有 `memory_search` 工具**（不需要）：发现靠清单文件，取内容靠 `memory_read`（与 Claude Code 一致——它也没有搜索记忆的工具，靠索引 + 普通读取）；
- `memory_read` 会返回该文件的 `updated` 时间 → 供「你读过的记忆被更新」提醒使用（你读过哪些文件是可追踪的）。

---

## 4. 记忆的「保质期」：置信度、衰减、淘汰

| 概念 | 规则（用户视角） |
|:--|:--|
| 置信度 1–3 | 3 = 你明确说过的（「以后都要…」）；2 = 否决/确认类；1 = 重复出现推断出来的（**模糊**） |
| **谁能看到** | **你（master）只看得到 confidence ≥ 2 的条目**；confidence = 1 的模糊条目**不进清单、不注入**，由 memory agent 独占管理（升级/合并/淘汰） |
| 升级 | 同一偏好再次出现 → 置信度取更高值（封顶 3）；升到 2 以上时由 memory agent 把它**移入正式清单** |
| 过期提醒 | 你说过「下周再看」这类话 → 到期时提醒你 |
| 冲突 | 同一 `subject` 新旧矛盾 → 新的生效，旧的标记为被取代（不物理删除，可查历史） |

**过时的记忆怎么消失（2026-09-13 补齐）**：不做"半衰期慢慢降权"这种公式（可见性随日期漂移，你无法解释为什么某条昨天还在、今天没了），
改成**三种确定 + 一种判断**的路径：

| 情形 | 谁来做 | 结果 |
|:--|:--|:--|
| 同主题出现**矛盾**新说法 | 归纳代理（代码自动取代） | 旧的标 `superseded`，**退出清单**，文件留作演化记录 |
| 某条**明显没用了**（话题早结束） | 归纳代理（每轮归纳时顺带判断） | 用 `confidence: 1` **软淘汰** → 掉回模糊池，你看不到了，但还能恢复 |
| 某条**是错的**、又没替代说法 | 归纳代理（`memory_forget`，每轮最多 3 条） | 写墓碑 + 退出清单（文件保留） |
| 你**明确说过**的条目（confidence 3）过时了 | **代码层禁止代理删除**（工具直接拒绝） | 只能由新说法取代，或你本人 `/memory forget <slug>` |

判断依据只有两样：清单里的 `updated`（`3 days ago` 这种时效提示）和**本轮对话内容**——也就是"它在相关话题里被重新提到时才有机会被纠正"，
不会出现"某天你的记忆被批量清空"。另外清单注入恒定 ≤800 tokens：即使有滞留条目，它们也只能挤占预算，不会撑爆上下文。

**还想更主动一点？已经做了（LRU 主动维护，2026-09-13）**：

记忆会**用进废退**，但用的是"离散档位"而不是时间衰减公式：

| 动作 | 什么条件 | 结果 |
|:--|:--|:--|
| 升一级 | 被**真正用过** 2 次（读全文 / 被重申）且最近还在用 | conf +1（最高 3），计数清零（要重新积累证据） |
| 降一级 | **90 个"活动日"**没被用到 | conf −1（最低 1），一次只降一级 |
| 换出 | 可见条目超过 **memory window**（默认 200 条）时，最久没用的被挤出去；或降级后跌破可见阈值 | 进候选池（你看不到）+ **开始销毁倒计时** |
| 复活 | 倒计时内又被用到（读到 / 被重申） | conf 拉回可见阈值 → 重回清单，倒计时清零 |
| 销毁 | 换出后 **30 个"活动日"**内一直没被用到 | 默认移到 `legacy/archive/`（**文件保留**）；配 `lru_destroy_mode = "delete"` 才物理删除 |
| 免疫 | 你 `pin` 过的条目 | 不升不降不换出不销毁 |

**关键：时间单位是"活动日"，不是日历天。** 活动日 = "这一天你确实用了程序"；你半年不开程序，
这些天**不推进老化** —— 否则一回来所有记忆会同时"过期 90 天"，被一次清空（这正是初版的问题）。

四个设计要点，都是为了让你能理解、能控制：

1. **"使用"只算两种**：模型真的 `memory_read` 读了全文，或者这条被写入/重申。**"出现在清单里"不算使用** —— 否则会变成"越注入越升级"的正反馈，那就毫无意义了。
2. **先换出、再倒计时、最后才销毁**：任何"离开你视线"的条目都会拿到一段**缓刑期**（默认 30 活动日），期间被再次用到就复活；到期默认也只是**归档**，文件仍在 `legacy/archive/` 里。
3. **可解释**：每次升降级/换出/复活/销毁都写进 `audit.jsonl`（带活动日序号与原因 window/decay/candidate）；`/memory show` 显示 `uses=N last-used=Nd ago pinned`；`/memory gc --dry-run` 先预览。
4. **给你逃生舱**：`/memory pin <slug>` 永久免疫，`lru_enabled = false` 全局关掉，阈值都可调。

触发时机：**每次开新会话时结算一次**（在构建清单之前，所以注入的清单一定是结算后的结果）+ `/memory gc` 手动。不常驻、不打扰、失败只写审计。

**候选池（`confidence: 1`）怎么升上来**：那批条目 master 看不到、不会被注入、也永远不会去读它，
所以它一度是"只进不出"。现在有三条通道：① **归纳代理**在输入里能看到候选池（含"共被使用 N 次"），
发现本轮又被印证 → 用它的 slug 更新并升到 2；② **系统兜底**：同主题每次重申/取代都累加计数
（跨取代延续），攒够 2 次就自动升到 2 —— 不依赖代理配合；③ 同义正文合并时取 max。
所以你不需要操心"模糊记忆会不会永远沉底"。（反之，长期没人提的候选条目会走上面的销毁倒计时，
默认 30 活动日 → 归档，候选池不会变成无限期坟场。）

**"想写新记忆时，模型会先看有没有语义接近的旧条目吗？"（会，两层保障）**：
归纳代理的输入开头固定有「现有记忆索引」（正式条目 + 候选池，含 slug 与 `共被使用 N 次`）；
再加一段「**可能与本轮相关**」—— 用确定性的关键词初筛（拉丁词 + 中文二元组，subject/tag 命中加权，
零额外 API 成本）把语义接近的几条顶到眼前。提示词要求它：**语义接近就先 `memory_read` 读全文**，
再决定 更新（同义）/ 取代（矛盾）/ 新建（无关）。这样既能减少重复条目，也不会因为"相似度阈值"误判合并。

**"候选/待销毁的条目，归纳 agent 还会管吗？会，而且只有它能救"**：
候选池（含倒计时中的）都列在它的输入索引里，倒计时条目标着 `⏳待销毁(已 N 活动日)`。
它若确认这条仍然有效 → 带 slug 升到 `confidence: 2` → **用后即复活**（不会先被杀掉）；
只读不写只算"看到"，会**推迟**倒计时但**不能复活**；确认无用则 `memory_forget` 立即淘汰。

**"换出到底是按时间窗统计频率，还是按容量？"** —— 现状是**"多久没用"为主 + "窗口容量"封顶**，
**没有"滑动窗口内统计频率"这层**；频率只在容量换出的排序并列时当 tie-break。
原因：一条记忆一生可能只被读几次，按窗口算频率全是噪声；而且计数在升降级时会清零，不是终生计数。
频率真正起作用的地方是**升级**（用得多 → 往上升 → 更抗老化）。
如果你想让"重度使用的记忆更抗老化"，可加"终生计数 × 老化窗口倍数"（设计稿 §4.3 方案 A，约 30 行），说一声就做。

**"confidence 1 和'待销毁'会不会打架？"（审过一次，改了一处）**：
两者确实**共用同一个状态**（conf 1 = 你看不到），但意图不同 —— 一个是**入口**（等被再次印证就升进来），
一个是**出口**（等到期就清掉）。用"是否已离开清单"来区分，`/memory candidates` 也分成两段显示。

审出的一处真问题：**晋升要求"重复出现"，但出生候选原本只活 90+30 个活动日** ——
"一年才提一次"的有效偏好会在第二次出现前就被清掉，下次只能重建（永远升不进清单）。
已修：**出生候选的期限单独设为 365 活动日**（候选池几乎不花成本，值得等），
而"曾经进过清单又被挤出来"的仍是 30 活动日。
另外两点是**刻意的**：被淘汰的条目**用过一次就能复活**（判决的依据本就是"没人用"，所以一次真实使用足以撤销），
而新候选要**两次**才算晋升；复活只回到"可用"档（2），想回到最高档要继续被使用 —— 避免"偶然读一次就恢复最高信任"。

**"待销毁"在磁盘上长什么样？** 条目文件本身**看不出来** —— 它只写 `confidence: 1` 与 `status: candidate`，
和"待观察"的新候选**完全同形**；区别在该层 `state.json` 的 `usage[slug].evictedAt`（＋`evictedDay/evictReason`），
这是唯一判据。好处是记忆内容与运行时统计分离、不会有两份真相；代价是**删掉/损坏 state.json 就"忘记"倒计时**
（那些条目回到"待观察"、不会被销毁 —— 失败方向是安全的）。想人工查看：`/memory candidates` 分了
「待观察 / ⏳待销毁（已 N/期限 活动日, 原因）」两段；`📌` 是用户钉住、免疫自动清理的条目。

**去重怎么做的（已简化）**：不再用「文本相似度算法」，而是**把现有清单喂给归纳 agent，让它自己判断「该更新哪条」**
（对齐 Claude Code 的做法）。代码侧只保留三条确定性规则：① 明确指定 id → 改写那一条；② 同 `subject` 矛盾 → 显式取代并留演化记录；
③ 归一化文本**完全相等** → 机械合并。另外用**游标**（`state.json` 记「已归纳到哪轮」）+「master 自己写过就跳过后台归纳」来避免重复归纳同一段对话
—— 重复的**源头**被掐掉，剩下的重复才交给模型判断。这样去重代码量从"阈值 + 打分 + 单测矩阵"降到"三条等值规则"。

---

## 5. 你能做什么

| 操作 | 命令 |
|:--|:--|
| 看状态（两层条数、开关、上次注入/归纳、可检索路径） | `/memory` |
| 列出条目（可按关键词、层级过滤） | `/memory show [kw] [--scope all|project|global]` |
| 看审计（哪次写了什么、为什么没注入） | `/memory show --audit` |
| 忘掉某条 | `/memory forget <id>` |
| 手动跑一次用进废退维护（先预览：`--dry-run`） | `/memory gc [--dry-run]` |
| 钉住某条使其免疫升降级/归档（取消：`unpin`） | `/memory pin <slug>` |
| 开关记忆 / 只关自动归纳 | `/memory on|off`、`/memory agent on|off` |
| 置顶某条（优先注入） | `/memory pin <id>` / `unpin <id>` |
| 启动时关闭 | `--no-memory`（还有 `--memory-scope`、`--memory-model`） |

**模型自己也能用记忆**：主 agent 有 `memory_search` 与 `memory_write` 两个工具（子代理没有）。
所以你可以直接说「记住：以后 X」或「查一下我之前说过的测试约定」。

---

## 6. 两个必须绕开的实现约束（你问到的 B、C）

### 约束 B：`search_content` 不遍历隐藏目录

搜索工具递归时会跳过名字以 `.` 开头的子目录（`src/tools/search-content.ts:63`），
所以从工作区根 `search_content("结论")` **搜不到** `.deepseek-arch/memory/` 里的东西。

- 这不是「有问题」，而是「不能依赖通用搜索」的依据：必须给专用工具 `memory_search`，并在注入块里写明可检索路径。
- 补充事实：显式指定 `path: ".deepseek-arch/memory"` 时**可以**搜到——被跳过的规则作用于「递归时的子目录名」，
  而搜索起点本身不受影响，且 memory 目录里的文件名不以 `.` 开头。
- 参考 claude-code：它不解决这个问题，因为它没有文件沙箱；它用「索引注入 + 让模型自己读文件」的方式绕过。

### 约束 C：配置合并是白名单

加载配置时只挑 `paths / defaults / providers / pricing / systemPrompts / display` 这几段（`src/core/config.ts:362-369`），
`set()` 也只认这几段（`src/core/config.ts:490-501`）。所以新增 `[memory]`/`[heartbeat]` 必须**同时改 5 处**，
否则用户会看到两种典型症状：

1. **配置里写了但不生效**：你在 `config.toml` 写 `[memory] enabled = false`，程序读出来仍是 `undefined` → 按默认值（开启）跑，
   你会觉得「我明明关了」；
2. **命令报错**：`/memory off` 内部要写回配置，`set('memory.enabled', ...)` 会直接抛「不支持的配置段」。

修法就是把这 5 处都补上（类型定义、`AppConfig`、`ResolvedConfig` 组装、`set()` 的 `fileMap`、默认配置模板），
设计稿 §9.3 给了精确清单。

---

## 7. session 里的 system-prompt.txt：重建还是复用？

**结论（已核对代码）：**

| 场景 | 行为 |
|:--|:--|
| 新建会话 | 构建一次 → 写一份到 `<session>/system-prompt.txt`（`src/core/session.ts:176-179`） |
| 每次请求 | 直接用内存里的那条 system 消息，**不重建、不重读文件** |
| `resume` | 读回 `system-prompt.txt` 并**覆盖**当前构建的（`src/core/session.ts:191-194`，为的是让消息前缀与缓存一致）→ 是复用，不是重建 |

副作用：如果你升级了软件（改了 `system_prompt.txt` / skills），**旧会话 resume 时仍然用旧的 system prompt**。

**你的提议（compact 后重建一次 system prompt）已升为默认项**（设计稿 §12 R7 + R11）：Claude Code 在 `/clear` 与 `/compact` 就是这么做的（清掉段落缓存重算）。要注意两点：
1. 重建后必须**同步重写 `<session>/system-prompt.txt`**，否则下次 resume 又变回旧的；
2. 重建意味着「同一会话前后两段用不同 system prompt」，这在 API 上完全合法（system 是消息数组首条），
   只是那一次请求的前缀缓存作废——compact 场景下本来就要作废，无额外代价。

另外多了一个**显式刷新**入口：`/memory refresh`（重建 system prompt + 重写快照），用于「新增了 skill / 记忆变了想立刻生效」的场景。
它是有代价的动作（作废一次前缀），所以做成手动而不是自动。

---

## 8. 与 claude-code 的对照（我们采用/不采用什么）

参考实现：`~/.claude/projects/<项目>/memory/`，核心是 `MEMORY.md` 索引 + 每主题一个 `.md`（带 frontmatter）+ `logs/yyyy/mm/dd.md` 日志，
索引被注入 system prompt，召回时让一个便宜模型从「文件名 + description 清单」里挑 ≤5 个文件再读全文。

| 维度 | claude-code | 本设计 | 取舍理由 |
|:--|:--|:--|:--|
| 存储形态 | 每主题一个 `.md` + `MEMORY.md` 索引 | **采用**（`index.md` + 主题 `.md` + `log/`） | 人可读可改、mtime 表达更新、agent 可直接读项目层 |
| 索引注入 | `MEMORY.md` 进 system prompt（限 200 行/25KB），**段落 memoize**，`/clear`·`/compact` 重算 | **采用**：概览进 system prompt，重装点 = 新会话 / compact / `/memory refresh` | system prompt 在会话内冻结 → 缓存零代价；只在"前缀本来就要变"的时刻重装 |
| 会话内变化 | 无（靠下一轮召回兜底） | 合并进当前发言的 `<system-reminder>` 小块（采纳其动态通道形态） | 你要的「变化提醒」；不新开 user 消息 |
| 召回 | 便宜模型（Sonnet）从清单里挑 ≤5 个文件 | **同款思路**，固定用 `deepseek-v4-flash`；清单在预算内时直接整份注入（零调用） | 你要的质量优先；但只在"超预算裁剪"和"会话内动态出示"两处出手 |
| 工具面 | 无 memory_search；靠 Grep/Glob/Read（无沙箱） | 无 `memory_search`；新增 **`memory_read`**（项目层 + 全局层，core 内直接读文件绕过沙箱） | 你有沙箱，全局层必须有专用通道（约束 A） |
| 更新表达 | 改写文件 + 改索引行 | 同（但写入统一走 `memory_write` 工具，保证格式与审计） | 防止模型手写坏 frontmatter |
| 时效提示 | `N days ago` + 陈旧告警注入 | **采用**（概览行带 `updated` 人话时间） | 模型对「3 天前」比 ISO 时间戳更有感觉 |
| 去重 | 已读过滤 + 已出示过滤（compact 后自然重置） | **采用** | 避免重复注入同一份内容 |
| 分类词表 | `user / feedback / project / reference` | 采用其**受控词表 + 不记什么清单** | 直接复用经过验证的提示词约束，省得自己踩坑 |
| 全局/项目 | 按项目分目录（+团队目录） | 两层：项目 `{workspace}/.deepseek-arch/memory/`、全局 `~/.deepseek-arch/memory/` | 你要的「跨项目偏好」需要全局层 |
| 沙箱 | 无文件沙箱 | 有沙箱 → 全局层必须走专用工具；项目层可用 `read_file` | 约束 A/B 的直接后果 |

---

## 9. 成本（为什么这么注入）

用仓库现有价格表（v4-pro：命中 0.2 / 未命中 6.0 CNY 每 1M tokens）：

| 做法 | 每次发言的额外输入成本 |
|:--|:--|
| **本设计**：概览进 system prompt（会话创建时一次，之后冻结）+ 会话内变化合并进你当前发言的 `<system-reminder>` | 概览：**≈0**（随会话前缀，冻结）；变化那次约 **¥0.005** |
| 会话中途自动重装 system prompt（本设计禁止） | 每次重装让**整段历史**作废（长会话 10 万 tokens）→ **¥0.6+/次** |
| 每轮把记忆追加在请求末尾 | 每个 agent 轮次都算未命中 → 10 轮约 **¥0.05**（差 10 倍） |

一句话：**概览进 system prompt 并冻结（零代价、只会过期）；变化走合并进当前发言的小块（只在真变化时花一次小钱）；重装只发生在"前缀本来就要变"的时刻。**

---

## 10. 落地清单（要动的模块）

> **实现状态（2026-09-13）：已全部落地并测试**（全量 569 测试通过）。逐模块状态与「未做项」见设计稿 §14。

| 模块 | 用途 | 状态 |
|:--|:--|:--|
| `src/core/memory-store.ts`（新） | 读/写主题 `.md`、生成/维护 `MEMORY.md`、置信度分层、去重/取代、审计 | ✅ |
| `src/core/memory-recall.ts`（新） | flash 召回选择（超预算裁剪），失败退化为按更新时间倒序 | ✅ |
| `src/core/memory-inject.ts`（新） | 清单块渲染、变化提醒（新增/更新/移除）、已读追踪、已出示去重 | ✅ |
| `src/core/memory-service.ts`（新） | 两层路径 + 实例装配（工具与 agent 共用） | ✅ |
| `src/core/memory-agent.ts` + `memory-agent-prompt.ts`（新） | 后台归纳代理（游标增量、主/后台互斥、watchdog、配额、失败只写审计） | ✅ |
| `src/tools/memory-read.ts` / `memory-write.ts`（新） | `memory_read` 读两层（绕过沙箱，全局层唯一通道）、`memory_write` 参数即字段（`scope` 选目录） | ✅ |
| `src/core/session.ts` | 清单注入 system prompt（会话创建）、resume 播种、变化提醒落盘、归纳触发、`refreshMemoryPrompt`、compact 后重建 | ✅ |
| `src/core/config.ts` + `src/types/config.ts` | `[memory]` 五处同步（约束 C） | ✅ |
| `src/presentation/tui-app.ts` | `/memory [show\|candidates\|forget\|on\|off\|refresh]`、写入/归纳提示行 | ✅ |
| `src/cli/index.ts` | 装配、`--no-memory`、headless「已更新记忆」走 stderr | ✅ |
| 衰减/LRU 淘汰、`remindAt` 到期提醒、`/memory show --audit`、`pin/unpin`、`logs/` 写入、**心跳** | — | ⏳ 见设计稿 §14.2 |

---

## 11. 还需要你拍板的点

**已全部确认并落地**（实施依据 = 设计稿 v2；实现状态见设计稿 §12）。

后续可选工作（已挂起，含现状/原因/工作量）：**`docs/todo/memory-open-items.md`** ——
包括 CLI 面补齐（`--quiet`/`pin`/`show --audit` 等）、衰减/LRU、跨层去重分层、`logs/` 写入、
`--prompt` 与心跳契约，以及**心跳机制**本身。

| 项 | 决定 |
|:--|:--|
| 置信度阈值 | **2**（你能看到「否决/确认」类；`confidence=1` 的模糊条目不可见） |
| memory agent 用的模型 | **`deepseek-v4-flash`**（与召回同一模型） |
| 模糊条目查看入口 | **提供** `/memory candidates`（只查看，管理仍归 memory agent） |
| 心跳机制 | **暂缓**，等 memory 主体完成后再做 |
| compact 后重建 system prompt | 本期必做（含重写快照） |
| 增量判断 / 去重简化 / 文件格式 / 召回用 flash / 无 memory_search / 注入落盘 / 游标 | 已定（见设计稿 §13） |

---

## 12. 附录：召回决策的两个方案（通俗对比）

### 场景

你有一条记忆：
```
description: 显示参数偏好 normal 模式（不要 detail）
tags: [display, config]
```
现在你说：**「帮我改一下配置的展示模式」**。系统要从记忆库里挑出「这条值得注入」。

### 方案 A：让一个便宜模型来挑（Claude Code 的做法）

1. 把记忆目录扫一遍，每条生成**一行清单**：`[类型] 文件名 (时间): description`；
2. 把你这句话 + 整份清单交给一个便宜模型，要求它「只挑确定有用的，最多 5 个，不确定就不要挑」；
3. 模型返回文件名数组（结构化输出，最多 256 tokens）；
4. 读这 5 个文件的全文，注入。

**一句话**：像"让实习生拿目录帮你找资料"——他懂你的意思，但每次都要叫他一次。

### 方案 B：本地算分排序（本设计）

1. 把你这句话切词，和每一条记忆的 `tags`／`paths`／正文关键词比对，得到「相关性」；
2. 再叠加：置信度（你说过的更重）、新旧（越近越重）、用过几次（LRU）；
3. 排序取前 K 条（≤10 条 / ≤800 tokens）。

**一句话**：像"用检索关键词 + 权重排序"——不要人工，快且每次结果一样。

### 对比

| 维度 | A 便宜模型挑 | B 本地算分 |
|:--|:--|:--|
| 每次都花钱吗 | 是（一次小模型调用） | **否**（纯本地计算） |
| 快吗 | 有网络往返（通常几百 ms，异步可掩盖） | **瞬时** |
| 同样输入，同样结果吗 | 不一定（模型有随机性） | **一定** |
| 能解释「为什么没选它」吗 | 只能看它的理由文本 | **能**：每条都有分数，可写进审计 |
| 懂不懂近义/换说法 | **懂**（"展示模式" ↔ "display"、"别铺垫" ↔ "结论先行"） | 只懂字面（要靠 tags/description 写好） |
| 挂了会怎样 | API 超时 → 本次啥也不注入 | 不依赖外部，不会挂 |
| 对缓存的影响 | 选出的集合容易变 → 注入内容抖动 | 集合稳定（对缓存友好） |

### 我们怎么选（已定：**用 A，且固定用 `deepseek-v4-flash`**）

- **发现通道不是"召回"**：清单文件 `MEMORY.md` **整份注入**（受预算约束）→ 模型看得到全部条目，
  想读细节就用 `memory_read`。**这一层不需要任何选择算法，也不需要 memory_search 工具。**
- **`deepseek-v4-flash` 只在两个地方出手**（配置 `memory.recall_model`）：
  1. **清单超预算时**：从清单里挑出该注入的相关行（其余仍留在文件里，模型可 `memory_read`）；
  2. **会话内动态出示**：判断"当前任务该不该主动出示某几条记忆的全文"。
  这两处才需要"懂语义"，也值得花一次便宜模型调用。
- **失败兜底**：flash 超时/失败 → 退化为「按 `updated` 倒序取预算内的行」并记审计；**不因为召回失败而少注入或不注入**。
- 原来的"本地算分"方案（B）**不再作为默认**，只在兜底与未来调试里保留思路（分母为零的场景没必要算分排序）。
