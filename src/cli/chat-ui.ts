...
export class ChatUI {
	 ...
	 *
	 * 所有输出合并为单次 process.stdout.write 调用，避免 Node.js TTY
	 * WritableStream 内部缓冲导致的刷新延迟。
	 */
	private drawStreamUpdate(): void {
		...
		for (let row = separatorRow; row < this.termHeight; row++) {
			parts.push(cursorTo(row, 0) + ERASE_LINE);
		}
		...
		if (this.uiState === 'sending') {
			...
			if (currentRow < this.termHeight) {
				parts.push(cursorTo(currentRow, 0) + chalk.gray(indicator));
				currentRow++;
			}
		...
		if (this.liveStream.reasoning) {
			...
			for (const line of reasoningLines) {
				...
				parts.push(cursorTo(currentRow, 0) + chalk.gray(line));
				currentRow++;
			}
		}
		...
		if (this.liveStream.content) {
			...
			for (const line of contentLines) {
				if (currentRow >= this.termHeight) break;
				parts.push(cursorTo(currentRow, 0) + chalk.white(line));
				currentRow++;
			}
		}
		...
		if (this.inputQueue.length > 0) {
			if (currentRow < this.termHeight) {
				parts.push(cursorTo(currentRow, 0) + chalk.dim(`⏳ 等待中 (${this.inputQueue.length} 条)...`));
				currentRow++;
			}
		}
