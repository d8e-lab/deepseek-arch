...
import { dim, cyan, padToWidth } from './ansi.js';
...
/** Selector 终端输出接口（由表示层注入，SDK 不直接操作终端） */
...
export class Selector<T> {
	...
	/** 终端输出接口（表示层注入） */
	private io: SelectorIO;

	constructor(items: SelectOption<T>[], io: SelectorIO, prompt?: string) {
		if (items.length === 0) {
	...
	private render(): void {
		...
		if (this.renderCount > 0) {
			this.io.write(`\x1b[${this.totalLines - 1}A`);
		}
		this.renderCount++;
		...
		for (let i = 0; i < this.items.length; i++) {
			...
			this.io.write(i === this.selectedIndex ? cyan(padded) : dim(padded));
