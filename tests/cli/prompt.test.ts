...
describe('chat --prompt（非交互单轮，e2e）', () => {
	...
	it('单轮执行：stdout 输出最终回复、退出码 0、会话正常落盘', () => {
		...
		try {
			const { stdout, status } = runWithEnv(['chat', '--mock', '--prompt', '测试'], { HOME: home });
	...
	it('--workspace 指向存在的目录：正常执行并退出 0', () => {
		...
		const ws = mkdtempSync(join(tmpdir(), 'deepseek-arch-ws-'));
		try {
	...
	it('--prompt 配合 --resume 续跑已存在的会话（轮次累加）', () => {
		...
		try {
			const env = { HOME: home };
			runWithEnv(['chat', '--mock', '--prompt', '测试'], env);

			const sessionsDir = join(home, '.deepseek-arch', 'sessions');
			const id = readdirSync(sessionsDir)[0];

			const { stdout, status } = runWithEnv(['chat', '--mock', '--prompt', '你是谁', '--resume', id], env);
			expect(status).toBe(0);
	...
	it('--resume 不存在的会话：退出 1 并提示', () => {
		...
		try {
			const { stderr, status } = runWithEnv(
				['chat', '--mock', '--prompt', '测试', '--resume', 'nonexistent-id'],
				{ HOME: home },
			);
			expect(status).toBe(1);
